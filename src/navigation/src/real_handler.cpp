...
class Handler{
	...
	public:
		Handler():nh("~"){
			...
			work=false;
			nh.getParam("lidar_topic_name", lidar_topic_name);
			...
			nh.getParam("in_row_server_name", in_row_server_name);
			nh.getParam("turn_server_name", turn_server_name);
			nh.getParam("end_row_server_name", end_row_server_name);
			nh.getParam("file_path", file_path);
			nh.getParam("storage", storage);
			end_of_row_result=1;
			turns_count=0;
		...
		void in_row_navigation_algo(){
			...
			if(path_from_file==false){
				...
				// read data for future navigation
				if(file_input.good()){
		...
		int end_of_row(){
			...
			if(turn(0, next_turning_side())){
				...
				if (end_client.start_client(n, 2.0, 2.0, end_row_server_name, read_tojump_rows)){
